...
function Category() {
  ...
  return (
    <div className="w-full bg-[#f1f2f4] min-h-screen pt-4 px-5 ">
      <div className="flex bg-white justify-evenly text-center px-5">
        {allCategoryData?.allCategory?.map((category) => {
          ...
          onMouseLeave={() => setHoveredCategory(null)}>
            ...
            <p className="font-semibold flex">{category.name}
              ...
              <div className="absolute left-[-10rem]  w-[15rem] bg-white text-black border rounded-lg shadow-lg text-left pl-2">

                {subcategories.map((item) =>(
                  <p key={item.id} className="text-sm py-3">{item.name}</p>
                ))}
              </div> }
          </div>
        )})}
